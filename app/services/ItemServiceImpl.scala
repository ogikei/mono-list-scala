...
class ItemServiceImpl @Inject()(
  ...
  override def getItemByCode(itemCode: String)(implicit dbSession: DBSession)
  : Future[Option[Item]] = Future {
  ...
  override def getItemAndCreateByCode(itemCode: String)(implicit dbSession: DBSession)
  : Future[Item] = {
  ...
  private def searchItemByItemCode(itemCode: String): Future[Item] =
    rakutenItemSearchAPI
        .searchItems(itemCode = Some(itemCode)).map(_.Items.head).map(createItemFromRakutenItem)
  ...
  def getItemsByRanking(`type`: WantHaveType.Value)(implicit dbSession: DBSession)
  : Try[Seq[(Item, Int)]] = Try {
    val i = Item.syntax("i")
